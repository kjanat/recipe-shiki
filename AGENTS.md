...
`@shikijs/langs/*` shape. Grammar JSON is inlined into `dist/index.js` at
build time — no runtime dep on `recipe-tmlanguage`.
...
- Grammar content lives in `recipe-tmlanguage`. This package MUST NOT
  duplicate vocab or patterns.
- Shiki-specific metadata (canonical `name: "recipe"`, `aliases`,
  `displayName`) lives in `src/index.ts`.
...
- tsdown inlines the JSON at build — `dist/index.js` has no runtime
  import of `recipe-tmlanguage`.
...
- Do not add runtime deps. This package should be build-only consumption
  of `recipe-tmlanguage` + peer `shiki`.
- Do not drop `aliases` without a major version bump — downstream code
  uses them.
- Do not add subpath exports unless Shiki grows multi-variant support
  for a single language.

## CHANGE CHECKLIST

- Upstream grammar changed: bump `recipe-tmlanguage` devDep, rebuild,
  diff `dist/index.js`, bump this package's version.
